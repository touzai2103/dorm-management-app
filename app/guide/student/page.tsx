...
const Badge = ({ color, label, description }: { color: string; label: string; description: string }) => (
  <div className="flex items-center gap-3">
    <span className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-base font-bold ${color}`}>
      {label}
    </span>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-4">
        <Section title="食事の申告方法">
          ...
          <Step num={2} text="「朝食」「夕食」のボタンをタップして切り替えます。赤くなったら「食べる」の申告です。" />
          <Step num={3} text="タップした時点で自動保存されます。保存ボタンは不要です。" />
        </Section>

        <Section title="マークの見方">
          <Badge color="bg-red-100 text-red-500" label="●" description="食べる" />
          <Badge color="bg-gray-100 text-gray-400" label="✕" description="食べない" />
        </Section>
