...
function calcGrade(enrollmentYear: number | null): number | null {
...
export default async function StudentDetailPage({
  ...
  return (
    <div className="min-h-screen bg-gray-50 animate-page-in">
      <KeyboardNav
        prevHref={prevStudent ? `/admin/students/${prevStudent.id}` : undefined}
        nextHref={nextStudent ? `/admin/students/${nextStudent.id}` : undefined}
      />
          ...
          <header className="bg-[#ebe7df] border-b border-[#d5cfc7] px-4 py-3 sticky top-4 z-10 flex items-center gap-3 shadow-sm rounded-xl">
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              {(() => {
                if (student.club === '教師') {
                  return (
                    <span className="text-xs rounded-full px-1.5 py-px font-medium shrink-0 bg-purple-100 text-purple-700">
                      教師
                    </span>
                  )
                }
            ...
            <div className="flex items-center gap-5 shrink-0">
              {prevStudent ? (
                <Link
                  ...
                  className="w-8 h-8 rounded-full bg-gray-900 flex items-center justify-center text-white hover:bg-gray-700 active:bg-gray-600 transition-colors"
                >
